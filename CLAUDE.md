...
**Category config (single source of truth):** `interfaces/categories.ts` defines `CATEGORIES`, a record keyed by category that holds label, emoji, switch color, JSON file name, and default visibility. The `CategoryKey` type, the `MarkerVisibility` map, the data loader, atom initial state, marker rendering, and the settings dialog are all derived from this object — adding a new category is a 2-step change (see below).
...
**State management:** Jotai atoms in `atoms/` handle three pieces of shared state:
...
- `MarkerVisibility` — `Record<CategoryKey, boolean>`
...
Only two changes are needed — everything else (types, atom state, marker rendering, settings UI) is derived from the config.
...
2. Add a new entry to `CATEGORIES` in `interfaces/categories.ts` (key, label, shortLabel, emoji, switchColor, dataFile, defaultVisible).
