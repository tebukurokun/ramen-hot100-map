...
- **`ShopMarker` の `position` は参照を安定させること（`useMemo`）。** 毎レンダー新しい配列を渡すと react-leaflet が `setLatLng` を呼び、markercluster がマーカーを付け外しして開いているポップアップが閉じる。
- **行きたい/行ったマーク（`shopMarksAtom`）の永続化キーは tabelog URL。** `shop.id` は index 連番でデータ更新時にずれ、`code` には年が入るため使わない（`utils/shops.ts` の `getShopMarkKey`）。
